...
export const BiblicalCard: React.FC<BiblicalCardProps> = ({ 
  ...
  return (
    <div className="flex flex-col items-center gap-6 animate-fade-in-up w-full px-4 md:px-0">
      {/* 
        CAPTURE TARGET
        Modern Cinematic Card Design - Clean & Sublime
        Responsive width: full width on mobile up to 340px
      */}
      <div className="relative group perspective-1000 w-full max-w-[300px] sm:max-w-[340px] mx-auto select-none">
        ...
        >
          ...
          <div className="absolute top-6 left-0 right-0 z-30 flex justify-center px-6">
             <span className="font-serif text-[#f2c46d] text-xs font-bold tracking-[0.2em] uppercase opacity-90 drop-shadow-md text-center">
                {title}
             </span>
          </div>

          {/* Bottom: Verse Text - Strictly in bottom half */}
          <div className="absolute bottom-0 left-0 right-0 h-[50%] z-30 flex flex-col justify-end p-6 md:p-8 pb-8 md:pb-10">
      ...
      <div className="flex flex-wrap justify-center items-center gap-3 bg-[#0A0C10] p-1.5 pr-2 rounded-full border border-gray-800 shadow-xl max-w-full">
        <button
          onClick={handleDownloadFull}
          disabled={isDownloading}
          className="flex items-center gap-2 px-5 py-2.5 bg-gray-100 hover:bg-white text-black text-sm font-bold rounded-full transition-all active:scale-95 disabled:opacity-50 shadow-[0_0_15px_rgba(255,255,255,0.1)] whitespace-nowrap"
        >
